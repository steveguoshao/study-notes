...
4.在/usr/local/zookeeper/目录下创建zookeeper用来存放数据的data目录,创建用来存放数据日志的logs目录

```
mkdir data && mkdir logs
```

5.在data目录下创建myid文件，由于是第一台服务器，所已写1
...
dataLogDir： 数据日志目录, 同样可以是任意目录. 如果没有设置该参数, 将使用和dataDir相同的设置
